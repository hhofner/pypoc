...
def run_multiple_chan_network_for(ground_nodes, air_nodes, space_nodes, t_steps, time_equivalent):
    ...
    ''' Create Channels'''
    channel_list = []
    channel_list.append(simple_channel.Channel(name='main1', time_step=time_equivalent, bandwidth=10e6, sinr=40))
    channel_list.append(simple_channel.Channel(name='main2', time_step=time_equivalent, bandwidth=10e6, sinr=40))
    channel_list.append(simple_channel.Channel(name='wimax', time_step=time_equivalent, bandwidth=10e3, sinr=40))
    ...
    level_one_relays = []
    for _ in range(air_nodes):
        new = generating_node.MultipleChannelNode(time_equivalent, packet_size=None, queue_size=5000, transmit_rate=np.random.normal(125000), gen_rate=0,
                                    type='lvl1', channel_interface_list=channel_list)
        level_one_relays.append(new)

    level_two_relays = []
    for _ in range(space_nodes):
        new = generating_node.MultipleChannelNode(time_equivalent, packet_size=None, queue_size=5000, transmit_rate=np.random.normal(1250000), gen_rate=0,
                                    type='lvl2', channel_interface_list=channel_list)
        level_two_relays.append(new)

    transmit_nodes = []
    for _ in range(ground_nodes - ground_nodes//2):
        new_node = generating_node.MultipleChannelNode(time_equivalent, packet_size=1, queue_size=int(8e7), transmit_rate=np.random.randint(125, 1250), gen_rate=np.random.normal(125),
...
data = []
for i in range(8):
    net_data = run_multiple_chan_network_for(4*i, 6*i, 2*i, 300, 0.001)
