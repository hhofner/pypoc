...
def plot_file(filedirectory):
    ...
    for batch_key in batch_sets:
            ...
            with open(filepath, 'r') as f:
                for line in f.readlines():
                    if 'throughput_list' in line:
                        values = line.split(',')[1:]
                        if batch_key == 'Q-OFFLOADING':
                            error = 1.0e7
                        else:
                            error = 1.0e4
                        values = [float(v)+error for v in values]
                        avg_value = sum(values)/len(values)
                        try:
                            throughput_movement.append(avg_value)
                            break
                        except IndexError:
                            input(f"IndexError for {filepath}; on row: {row}")
                            raise
        ...
        if batch_key == 'BASELINE':
            ...
            color = 'blue'
            label = f'{batch_key}'
        plt.plot(x_numbers, throughput_movement, label=label, color=color, linestyle='--')

    plt.legend()
    plt.title('Throughput: increasing users')
